...
string sol16v1(string solutionFileName)
{
	...
	start.push_back(0);
	DirectedNode startNode(start, 0, 0);
	...
	vector<DirectedNode> bestPath = GetShortestPath<DirectedNode>(lines, startNode, endNode, CreateNode, IsAtObjective, SimpleGetNeighbours2D, &visited);
	...
	vector<size_t> end)
{
	...
	return DirectedNode(position, cost, heuristic, previousNode.position);
}
